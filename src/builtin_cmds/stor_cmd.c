...
static void recieve_file(char *filename, connex_t *user_connex, int client_fd);

static int is_validpath(char *path, connex_t *user_connex);
...
void stor_cmd(int fd, command_t *cmd)
{
    ...
    recieve_file(new_path, user_connex, fd);
}
...
static void recieve_file(char *filename, connex_t *user_connex, int client_fd)
{
    ...
    if (pid == 0) {
        if (user_connex->type == PASSIVE)
            recvfile_pasv(filename, user_connex, client_fd);
        else if (user_connex->type == ACTIVE)
            recvfile_act(filename, user_connex, client_fd);
        exit(0);
    } else {
        if (user_connex->type == PASSIVE)
...
static int is_validpath(char *path, connex_t *user_connex)
{
    ...
    if (strncmp(ROOT_DIR, new_path, strlen(ROOT_DIR)) == 0)
        return (1);
    return (0);
}
...
static int contains_errors(int fd, connex_t *user_connex, command_t *cmd)
{
    ...
    if (strcmp("", cmd->args) == 0) {
        user_connex->response = "501 Syntax error in parameters.\n";
        send_all(fd, user_connex->response, strlen(user_connex->response));
        return (1);
    }
    if (!is_validpath(cmd->args, user_connex)) {
