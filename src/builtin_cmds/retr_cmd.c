...
static void transfer_file(char *filename, connex_t *user_connex, int client_fd);
...
void retr_cmd(int fd, command_t *cmd)
{
    ...
    transfer_file(new_path, user_connex, fd);
}
...
static void transfer_file(char *filename, connex_t *user_connex, int client_fd)
{
    ...
    if (pid == 0) {
        if (user_connex->type == PASSIVE)
            sendfile_pasv(filename, user_connex, client_fd);
        else if (user_connex->type == ACTIVE)
            sendfile_act(filename, user_connex, client_fd);
        exit(0);
    } else {
        if (user_connex->type == PASSIVE)
